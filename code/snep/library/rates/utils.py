...
def compute_vm_array(scale: float, n: int, m: int=0):
    m_was_none = m < 1
    if m_was_none:
        m = n
    pi_scale = 1. / (np.pi * scale) ** 2
    d_min, d_max = -np.pi, np.pi
    pos_pre = np.linspace(d_min, d_max, m, endpoint=False)
    pos_post = np.linspace(d_min, d_max, n, endpoint=False)
    cp = np.zeros((n, m), dtype=dtype)
    for i in range(n):
        d_curr = pos_post[i] - pos_pre
        d_curr[d_curr < d_min] -= 2*d_min
        d_curr[d_curr > d_max] -= 2*d_max
        ...
        cp[i, :] = ps
    if m_was_none:
        cp[np.eye(n, dtype=np.bool)] = 0
    cp /= cp.max()
    return cp


def compute_connections_fixed_in_out(J: dtype, p: dtype, n: int,
                                     m: int=0, swap_factor: int=8, max_dist: int=0, conn_prob: np.array=None):
    from scipy.sparse import coo_matrix
    c = boolean_fixed_in_out(p, n, m, swap_factor, max_dist, conn_prob)
    coo = coo_matrix(c.astype(dtype=dtype))
    coo.data.fill(J)
    ...
    if PLOT_CONNECTIONS:
        plt.figure()
        plt.imshow(c)
    return connection_sparse(csr_indices(indptr, indices, data), row_ind, col_ind, None)


def boolean_fixed_in_out(p: dtype, n: int, m: int=0, swap_factor: int=8, max_dist_n: int=0, conn_prob: np.array=None):
    """
    Computes connection matrix `c` with fixed in- and out-degree.
    :param p: Connection probability.
    :param n: Size of post-synaptic population.
    :param m: Size of pre-synaptic population. If `m==0` then connection is recurrent
                and `c[i,i] == 0`, unless `conn_prob[i,i] > 0`.
    :param swap_factor: Try to randomly swap `n*m*swap_factor` pairs of connections.
    :param max_dist_n: Connection in row `q` will only be swapped with connections in row `q` plus/minus `max_dist_n`.
                        If it is non-zero and `conn_prob is None`, a ring-network is constructed using
                        this as the connection distance.
    :param conn_prob: A matrix of shape (n,m) where `conn_prob[i,j]` is the probability of
                        a connection existing from j to i.
    :return: A boolean matrix of shape (n,m) which is true where a connection exists.
    """
    import matplotlib.animation as animation
    do_animate = False

    m_was_none = m == 0
    if m_was_none:  # Recurrent connection
        m = n
        if conn_prob is not None:
            cp = conn_prob
        elif max_dist_n < 1:
            cp = np.ones((n, n), dtype=dtype)
            cp[np.eye(n, dtype=np.bool)] = 0
        else:
            cp = np.zeros((n, n), dtype=dtype)
            pre = np.zeros(n, dtype=dtype)
            pre[1: max_dist_n + 1] = 1
            pre[-max_dist_n:] = 1
            assert pre.sum() == 2*max_dist_n
            for i in range(n):
                cp[i, :] = np.roll(pre, i)
    else:  # Between two distinct populations
        if conn_prob is not None:
            cp = conn_prob
        elif max_dist_n < 1:
            cp = np.ones((n, m), dtype=dtype)
        else:
            max_dist = max_dist_n * (m // n if m > n else 1)
            a, b = (n, m) if m > n else (m, n)
            cp = np.zeros((a, b), dtype=dtype)
            pre = np.zeros(b, dtype=dtype)
            pre[:(max_dist+1) + 1] = 1
            pre[-(max_dist-1):] = 1
            assert pre.sum() == max_dist*2 + 1
            for i in range(a):
                cp[i, :] = np.roll(pre, i*b//a)
            if n > m:
                cp = cp.T.copy()

    def can_swap(p_thresh, ra, c0, rb, c1):
        # These are the connections that will be created if the swap occurs. Only complete the swap if the total
        # probability is greater than p_thresh. We don't care about the probability of the removed connections,
        # especially because they might have been created during initialisation at some very low-probability location.
        return cp[ra, c0] * cp[rb, c1] >= p_thresh

    f = np.int(1/p)
    assert n % f == 0 and m % f == 0, f'{n}x{m} matrix cannot have fixed in- and out-degree when p={p}'

    k_cols = np.int(np.round(n * p))
    k_rows = m*k_cols//n
    c = np.zeros((n, m), dtype=np.bool)
    for i in range(0, f):
        c[i*k_cols:i*k_cols+k_cols, i*k_rows:i*k_rows+k_rows] = 1

    if m_was_none:  # Remove autapses for recurrent connection
        k_rows = k_cols = k_rows - 1
        c[np.eye(n, dtype=np.bool)] = 0

    swap_rows = np.random.randint(0, n, (n*m*swap_factor, 2))
    if max_dist_n > 0:
        swap_rows[:, 1] = swap_rows[:, 0]+np.random.randint(-max_dist_n, max_dist_n + 1, swap_rows.shape[0])
        ltz = 0 > swap_rows[:, 1]
        swap_rows[ltz, 1] = swap_rows[ltz, 1] + n
        gtn = n <= swap_rows[:, 1]
        swap_rows[gtn, 1] = swap_rows[gtn, 1] - n

    swap_rows = swap_rows[swap_rows[:, 0] != swap_rows[:, 1]]
    max_swaps = swap_rows.shape[0]
    p_thresh = np.random.uniform(0, 1, max_swaps)

    def try_swap(swap_idx, n_swaps):
        row_a, row_b = swap_rows[swap_idx, :]
        cols_oz = np.argwhere(np.logical_and(c[row_a, :], np.logical_not(c[row_b, :])))
        cols_zo = np.argwhere(np.logical_and(np.logical_not(c[row_a, :]), c[row_b, :]))
        if cols_oz.size > 0 and cols_zo.size > 0:
            col_oz = cols_oz[np.random.randint(0, cols_oz.size, 1)]
            col_zo = cols_zo[np.random.randint(0, cols_zo.size, 1)]
            if can_swap(p_thresh[swap_idx], row_a, col_zo, row_b, col_oz):
                c[row_a, col_oz] = c[row_b, col_zo] = 0
                c[row_a, col_zo] = c[row_b, col_oz] = 1
                n_swaps += 1

    n_swaps = np.zeros(1, dtype=np.int)
    if do_animate:
        def func(swap_idx, n_swaps, im):
            try_swap(swap_idx, n_swaps)
            im.set_data(c)
            return im
        fig = plt.figure()
        ax = fig.add_subplot(111)
        im = ax.imshow(c, aspect=m/n)
        _ = animation.FuncAnimation(fig, func, fargs=(n_swaps, im), frames=(max_swaps-1), interval=10)
        plt.show()
    else:
    ...
    n_swaps = n_swaps[0]
    fraction_swaps = 100*(n_swaps / max_swaps if max_swaps > 0 else 1)
    print(f'n={n}, p={p:.2f}, sf={swap_factor}, Performed {n_swaps:.1e} of '
          f'{max_swaps:.1e} attempted swaps ({fraction_swaps:3.1f}%)')
    assert np.all(c.sum(axis=0) == k_cols), 'Wrong out-degree for some neurons!'
    assert np.all(c.sum(axis=1) == k_rows), 'Wrong in-degree for some neurons!'
    if m_was_none:
        assert not np.diag(c).any(), 'Some autapses were created!'
    return c
...
def remake_connections(sparse: bool, row_ind: np.ndarray, col_ind: np.ndarray,
                       N_pre: int, N_post: int, J, scalar=False) -> Union[csr_indices, np.ndarray]:
    if sparse:
...
def _remake_connections_sparse(row_ind: np.ndarray, col_ind: np.ndarray,
                               N_pre: int, N_post: int, J, scalar) -> csr_indices:
    from scipy.sparse import coo_matrix
...
def _remake_connections_dense(row_ind: np.ndarray, col_ind: np.ndarray,
                              N_pre: int, N_post: int, J) -> np.ndarray:
    from scipy.sparse import coo_matrix
    if np.isscalar(J):
        data = np.empty(row_ind.size, dtype=dtype)
        data.fill(J)
...
def soft_plus_soft_max(alpha, max_y, x):
    """
    Soft-rectifies x, and uses the same sharpness to clip the upper bound as well
    :param alpha: Sharpness of non-linearity
    :param max_y: Maximum output value
    :param x: Input values
    :return: x transformed such that 0 < x < max_y
    """
    sp0 = np.log1p(np.exp(alpha * x))
    sp1 = np.log1p(np.exp(alpha * (x - max_y)))
    inf_idx = np.logical_not(np.isfinite(sp0))
    sp0[inf_idx] = x[inf_idx] * alpha
    inf_idx = np.logical_not(np.isfinite(sp1))
    sp1[inf_idx] = (x[inf_idx] - max_y) * alpha
