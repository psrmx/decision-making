...
class ExperimentTables(NetworkTables, ResultsTables, ParameterSpaceTables):
    ...
    def param_range_helpers(self, types, formats, filter_by=None, onlyfinished=False, onlyunfinished=False, defaults=None):
        ...
        for t in types:
            ...
            if t not in ignore_ranges:
                ...
                if len(set(values)) != n:
                    ...
                    t_map = {v: i for i, v in enumerate(values)}
                range_attributes[t] = param_range(n, np.array(values), np.array(names), t_map)
        for t, v in list(defaults.items()):
            if t in filter_by:
                del defaults[t]
                pr = param_ranges[t]
                fv = filter_by[t]
                if isinstance(pr, ParametersNamed):
                    ...
                    raise TypeError('Cannot handle given type: {}'.format(type(pr)))
                if not callable(fv):
                    if isinstance(fv, (np.float, np.float32, np.float64)):
                        fv = partial(np.isclose, fv, atol=1e-10)
                    else:
                        fv = partial(eq, fv)
                names = [n for n, v in zip(names, values) if fv(v)]
                values = [v for v in values if fv(v)]
                t_map = {v: i for i, v in enumerate(values)}
                range_attributes[t] = param_range(len(values), np.array(values), np.array(names), t_map)
            else:
        ...
        all_values = {}
        for t in list(param_ranges.keys()) + list(defaults.keys()):
