...
def user_select_experiment_dir(get_task_dir):
    import os
...
def experiment_opener(file_names, load_path, show=False, save=False, dpi=None, onlyfinished=False, onlyunfinished=False):
    def inner(func):
        def wrapped(*args, **kwargs):
            ...
            for f in files:
                ...
                # m = "stroke-linecap:butt;stroke-linejoin:round;stroke-miterlimit:"
                # f_ext = os.path.splitext(file_name)
                # if '.svg' == f_ext[1]:
                #     try:
                #         with open(file_name, 'r') as file_i:
                #             all_lines = [l.replace('100000', '10') if m in l else l for l in file_i]
                #         with open(file_name, 'w') as file_j:
                #             file_j.writelines(all_lines)
                #     except TypeError as e:
                #         print(e)
                #     except IOError as e:
                #         print(e)
                # print('Finished fixing miter limit')
            if show:
                plt.show()
...
def filter_tasks(task_ids, targets):
    x = [tid for t in targets
         for tid in task_ids
         if all(np.isclose(tid[k].value, v, atol=1e-10)
                if isinstance(v, (np.float, np.float32, np.float64)) else tid[k].value == v
                for k, v in t.items())]
    print('Filtered from {} to {} task ids, using {}'.format(len(task_ids), len(x), targets))
    return x


def allocate_aligned(shape, alignment=64, dtype=np.float32):
    size = np.prod(shape)
...
def reallocate_aligned(a, alignment=64):
...
def set_im(ax, ax_title, x_n, y_n, x_names, y_names, x_label, y_label):
