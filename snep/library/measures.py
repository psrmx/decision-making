...
def compute_state_variable(func, statevars, statevar_names, populations=None):
    ...
    for popname, neuron_idx in iteritems(populations):
...
def compute_coherence_interval_old(spiketimes, Tstart, Tend, tau):
    ...
    for i, spikes in iteritems(spiketimes):
...
def compute_coherence_interval(spiketimes, Tstart, Tend, tau):
    ...
    compiler_args = ['-w', '-Ofast', '-march=native']  # ['-O3']  # was
    support_code = '''
#include <cmath>
#define NPY_NO_DEPRECATED_API NPY_1_10_API_VERSION'''
    ...
    weave.inline(code, ps, support_code=support_code, type_converters=blitz, extra_compile_args=compiler_args)
    ...
    '''
    ps = ['N', 'z', 'kappa_num']
    kappa = weave.inline(code, ps, support_code=support_code, type_converters=blitz, extra_compile_args=compiler_args)
...
def compute_spike_vector_strength(all_spikes, timebins, vector):
    ...
    M = vector.size 
    X = np.empty((N,M))
    for i, spikes in iteritems(all_spikes):
...
def CV_ISI(spiketimes, minspikes=5, sortspikes=False):
    '''
...
def CV_ISI_population(spiketimes, timeL=0., timeR=np.infty, sortspikes=False):
    from scipy.stats import nanmean
    return nanmean([CV_ISI(spks[np.logical_and(timeL <= spks, spks < timeR)], sortspikes=sortspikes)
                    for spks in itervalues(spiketimes)])
...
def compute_firing_rates(spikes, timebins, size):
    bins = np.array([0., timebins]) if np.isscalar(timebins) else timebins
    diffs = np.diff(bins)
    rates = np.empty((size, diffs.size))
    for i in xrange(size):
        rates[i, :] = np.histogram(spikes[i], bins)[0]/diffs if i in spikes else 0
    return rates
